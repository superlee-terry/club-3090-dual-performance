...
class OpenAIServing:
    ...
    def _parse_tool_calls_from_content(
        ...
        if (
            ...
            # "required" with standard JSON-based parsing first, then fall
            # back to the configured parser for non-JSON formats such as
            # Qwen3-Coder XML tool-call blocks.
            tool_calls = []
            content = content or ""
            try:
            ...
            # club-3090 PR #35936 rebase: when tool_choice == "required", vLLM
            # forces structured JSON output regardless of which parser is
            # configured. The configured parser (e.g. qwen3_coder XML) won't
            # see its expected tokens. Try JSON validation first; fall back to
            # the parser on failure so XML-style outputs still work.
            if request.tool_choice == "required":
                try:
                    json_tool_calls = TypeAdapter(
                        ...
                        function_calls.append(
                            FunctionCall(
                                ...
                                arguments=json.dumps(
                                    tc.parameters, ensure_ascii=False
                                ),
                            )
                        )
