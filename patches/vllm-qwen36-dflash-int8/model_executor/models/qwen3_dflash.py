...
class DFlashQwen3Model(nn.Module):
    def __init__(
        ...
        *,
        ...
        # PR #42102 fix #2: DFlash drafter must use BF16 KV even when target
        # uses quantized KV (INT8 PTH / FP8). The drafter has independent KV
        # pools (partitioned in kv_cache_utils.py), so it doesn't need the
        # target's quantized dtype.
        from dataclasses import replace as _dc_replace
        draft_cache_config = current_vllm_config.cache_config
        if draft_cache_config is not None and draft_cache_config.cache_dtype != "auto":
            draft_cache_config = _dc_replace(draft_cache_config, cache_dtype="auto")
